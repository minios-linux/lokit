...
func (rt *ResolvedTarget) DocsPOPath(lang string) string {
	paths := rt.DocsPOPaths(lang)
	if len(paths) > 0 {
		return paths[0]
	}
	cfgDir := filepath.Dir(rt.AbsPo4aConfig())
	preferred := poLocalePreferred(lang)
	return filepath.Join(cfgDir, "po", preferred, preferred+".po")
}

// Po4aPOFile describes one PO file generated by a po4a master document.
type Po4aPOFile struct {
	Master string
	Path   string
}
...
func (rt *ResolvedTarget) DocsPOFiles(lang string) []Po4aPOFile {
	...
	cfg, _ := parsePo4aConfig(cfgPath)

	if len(cfg.Masters) > 0 && cfg.POPathPattern != "" {
		var out []Po4aPOFile
		for _, master := range cfg.Masters {
			for _, candidate := range poLocaleCandidates(lang) {
				rel := strings.ReplaceAll(cfg.POPathPattern, "$lang", candidate)
				rel = strings.ReplaceAll(rel, "$master", master)
				path := filepath.Join(cfgDir, filepath.FromSlash(rel))
				if _, err := os.Stat(path); err == nil {
					out = append(out, Po4aPOFile{Master: master, Path: path})
					break
				}
			}
		}
		...
		if entries, err := os.ReadDir(langDir); err == nil {
			...
			for _, entry := range entries {
				if strings.HasSuffix(entry.Name(), ".po") && !entry.IsDir() {
					master := strings.TrimSuffix(entry.Name(), ".po")
					out = append(out, Po4aPOFile{Master: master, Path: filepath.Join(langDir, entry.Name())})
				}
			}
			if len(out) > 0 {
...
// DocsPOPaths returns all .po file paths for a language in a po4a target.
func (rt *ResolvedTarget) DocsPOPaths(lang string) []string {
...
func parsePo4aLangs(cfgPath string) []string {
	...
	if err != nil || len(cfg.Langs) == 0 {
		return nil
	}
	langs := append([]string(nil), cfg.Langs...)
	sort.Strings(langs)
	return langs
}

type po4aConfig struct {
	Langs         []string
	POPathPattern string
	Masters       []string
}

func parsePo4aConfig(cfgPath string) (po4aConfig, error) {
	data, err := os.ReadFile(cfgPath)
	if err != nil {
		return po4aConfig{}, err
	}
	...
	for _, line := range strings.Split(string(data), "\n") {
		line = strings.TrimSpace(stripPo4aComment(line))
		if line == "" {
			continue
		}
		if strings.HasPrefix(line, "[po4a_langs]") {
			...
			if langsStr != "" {
				cfg.Langs = strings.Fields(langsStr)
			}
			continue
		}
		if strings.HasPrefix(line, "[po4a_paths]") {
			fields := strings.Fields(strings.TrimSpace(strings.TrimPrefix(line, "[po4a_paths]")))
			for _, field := range fields {
				if strings.HasPrefix(field, "$lang:") {
					cfg.POPathPattern = strings.TrimPrefix(field, "$lang:")
					break
				}
			}
			continue
...
func stripPo4aComment(line string) string {
